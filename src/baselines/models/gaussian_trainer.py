...
Original implementation licensed under the Apache License 2.0.
...
from src.baselines.models.gaussian_internal.perplexity import (
    calculate_gaussianlda_avg_ll,
)
from src.baselines.models.gaussian_internal.prior import Wishart
from src.baselines.models.gaussian_internal.utils import (
    chol_rank1_downdate,
    chol_rank1_update,
    get_logger,
    get_progress_bar,
...
class GaussianIterationDiagnostics:
    iteration: int
    sampling_sec: float
...
class GaussianLDATrainer:
    def __init__(
        ...
        self.table_density_kernel_backend = (
            GAUSSIAN_TABLE_DENSITY_BACKEND if self.cholesky_decomp else "python"
        )
        self.posterior_sampling_kernel_backend = (
            GAUSSIAN_POSTERIOR_SAMPLING_BACKEND if self.cholesky_decomp else "python"
        )
        self.avg_ll_kernel_backend = (
            GAUSSIAN_AVG_LL_BACKEND if self.cholesky_decomp else "python"
        )
        ...
        self._gaussian_nu = np.zeros(self.num_tables, dtype=np.float64)
        self._scaled_table_cholesky_ltriangular_mat = (
            np.zeros(
                (self.num_tables, self.embedding_size, self.embedding_size),
                dtype=np.float64,
            )
    ...
    def initialize(self):
        ...
        ave_ll = calculate_gaussianlda_avg_ll(
            get_progress_bar(len(self.corpus))(self.corpus),
            self.table_assignments,
            self.vocab_embeddings,
            self.table_means,
            self.table_cholesky_ltriangular_mat,
            self.prior,
            self.table_counts_per_doc,
        )
    ...
    def _refresh_density_caches(self) -> None:
        ...
        self._scaled_table_cholesky_ltriangular_mat = build_scaled_cholesky(
            table_counts=self.table_counts,
    ...
    def _refresh_table_density_cache(self, table_id: int) -> None:
        ...
        self._gaussian_nu[table_id] = (
            float(self.prior.nu)
            + float(self.table_counts[table_id])
            - float(self.embedding_size)
            + 1.0
        )
        count = float(self.table_counts[table_id])
        k_n = float(self.prior.kappa) + count
        nu_n = float(self.prior.nu) + count
        scale_tdistrn = np.sqrt(
            (k_n + 1.0) / (k_n * (nu_n - float(self.embedding_size) + 1.0))
        )
        self._scaled_table_cholesky_ltriangular_mat[table_id] = (
            scale_tdistrn * self.table_cholesky_ltriangular_mat[table_id]
        )
    ...
    def calculate_table_params(self, table_id):
        ...
        # Update table mean
        mu_n = (
            self.sum_table_customers[table_id] + self.prior.mu * self.prior.kappa
        ) / k_n
        ...
        scaleTdistrn = (k_n + 1.0) / (k_n * (nu_n - self.embedding_size + 1.0))
        scaled_sigma_n = (
            self.prior.sigma
            + self.sum_squared_table_customers[table_id]
            - mu_n_mu_nT
            + self.k0mu0mu0T
        )
        ...
        if sign_det_sig != 1:
            self.log.warn(
                "Error computing determinant of: {}. Table count = {}".format(
                    sigma_n, count
                )
            )
            raise ValueError(
                "sign of log determinant of sigma is {}".format(sign_det_sig)
            )
    ...
    def set_table_parameters(self, table_id):
        ...
        """
        mu, sigma, inv_sigma, log_det_sig, scaled_sigma = self.calculate_table_params(
            table_id
        )
    ...
    def update_table_params_chol(self, table_id, cust_id, is_removed=False):
        ...
        k_n = self.prior.kappa + count
        nu_n = self.prior.nu + count
        scaleTdistrn = (k_n + 1.0) / (k_n * (float(nu_n) - self.embedding_size + 1.0))
        ...
        if is_removed:
            ...
            x = (self.vocab_embeddings[cust_id] - self.table_means[table_id]) * np.sqrt(
                (k_n + 1.0) / k_n
            )
            ...
            # Update the mean
            new_mean = self.table_means[table_id] * (k_n + 1.0)
        ...
        self.log_determinants[table_id] = (
            ...
            + self.embedding_size * np.log(scaleTdistrn) / 2.0
        )
        self._refresh_table_density_cache(table_id)
    ...
    def log_multivariate_tdensity(self, x, table_id):
        ...
        nu = self.prior.nu + count - self.embedding_size + 1.0
        x_minus_mu = x - mu
        ...
        # Should be Nx1
        logprob = gammaln((nu + self.embedding_size) / 2.0) - (
            gammaln(nu / 2.0)
            + self.embedding_size / 2.0 * (np.log(nu) + np.log(math.pi))
            + 0.5 * log_detr
            + (nu + self.embedding_size) / 2.0 * np.log(1.0 + prod1 / nu)
        )
        return logprob
    ...
    def log_multivariate_tdensity_tables(self, x):
        ...
        # Now calculate the likelihood
        # calculate degrees of freedom of the T-distribution
        nu = self.prior.nu + count - self.embedding_size + 1.0  # (K,)'
    ...
    def sample(self, num_iterations):
        ...
        for iteration in range(num_iterations):
            ...
            for d, doc in enumerate(pbar(self.corpus)):
                if (
                    self.show_topics is not None
                    and self.show_topics > 0
                    and d % self.show_topics == 0
                ):
                    print("Topics after {:,} docs".format(d))
                    print(self.format_topics())

                uniforms = np.random.random(len(doc)).astype(np.float64, copy=False)
                for w, cust_id in enumerate(doc):
                    ...
                    self.table_assignments[d][
                        ...
                        -1
                    )  # Doesn't really make any difference, as only counts are used
                    ...
                    if self.cholesky_decomp:
                        ...
                        self.update_table_params_chol(
                            old_table_id, cust_id, is_removed=True
                        )
                    else:
                    ...
                    new_table_id = sample_topic_assignment(
                        self.table_counts_per_doc[:, d],
                        log_lls,
                        alpha=self.alpha,
                        uniform=float(uniforms[w]),
                    )
            ...
            sampling_sec = time.perf_counter() - sampling_start
            self.log.info("Computing average LL")
    ...
    def check_everything(
        self, iteration=None, doc_num=None, word_num=None, mid_sample=False
    ):
        """
        ...
        """
        warnings.warn(
            "Checking table counts in full: this should only be done for debugging purposes"
        )
        if iteration is not None and doc_num is not None and word_num is not None:
            mess = "\nIteration {:,}: doc {:,}. word {}{}".format(
                ...
                doc_num,
                word_num,
                " (after unsampling)" if mid_sample else "",
            )
        else:
            mess = ""
        ...
        for doc_num, doc in enumerate(self.table_assignments):
            ...
            if not np.all(
                table_counts_for_doc == self.table_counts_per_doc[:, doc_num]
            ):
                raise ValueError(
                    "DEBUG: table counts don't match for doc {}: {} != {}".format(
                        doc_num,
                        table_counts_for_doc,
                        self.table_counts_per_doc[:, doc_num],
                    )
                )
            table_counts += table_counts_for_doc
        if not np.all(table_counts == self.table_counts):
            raise ValueError(
                "table counts for each doc correct, but overall counts wrong: {} != {}{}".format(
        ...
        for table, customers in enumerate(table_ids):
            ...
            if diff > 1e-3:
                raise ValueError(
                    "Stored and computed vector sums for table {} don't match:\n{}\n{}\nDiff per dim: {}{}".format(
                        table,
                        sum_customers,
                        self.sum_table_customers[table],
                        diff,
                        mess,
                    )
                )
            # And for the squares
            sum_squared_customers = np.zeros(
                (self.embedding_size, self.embedding_size), dtype=np.float64
            )
            for cust in customers:
                sum_squared_customers += np.outer(
                    self.vocab_embeddings[cust], self.vocab_embeddings[cust]
                )
            diff = np.mean(
                np.abs(sum_squared_customers - self.sum_squared_table_customers[table])
            )
            if diff > 1e-3:
                raise ValueError(
                    "Stored and computed vector square sums for table {} don't match:\n{}\n{}\nDiff per dim: {}{}".format(
                        table,
                        sum_squared_customers,
                        self.sum_squared_table_customers[table],
                        diff,
                        mess,
                    )
                )
            ...
            table_mean = (sum_customers + self.prior.mu * self.prior.kappa) / (
                len(customers) + self.prior.kappa
            )
            # Compare to the stored mean
            diff = np.mean(np.abs(table_mean - self.table_means[table]))
            if diff > 1e-3:
                raise ValueError(
                    "Stored and computed means for table {} don't match:\n{}\n{}\nDiff per dim: {}{}".format(
            ...
            if self.cholesky_decomp:
                ...
                if diff > 1e-3:
                    raise ValueError(
                        "Cholesky decomposition of cov mat and iteratively updated "
