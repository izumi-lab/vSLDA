...
from src.baselines.models.gaussian_internal.perplexity import (
    ...
    calculate_sentence_gaussianlda_avg_ll_from_encoded,
)
...
class GaussianLDATrainer:
    def __init__(
        ...
        doc_means = []
        if self.preencode_corpus:
            for doc_embeddings in self.encoded_corpus:
                if doc_embeddings.size == 0:
                    continue
                doc_means.append(doc_embeddings.mean(axis=0))
        else:
            for doc in self.corpus:
                enc = np.asarray(self.encoder.encode(doc), dtype=np.float64)
                if enc.size == 0:
                    continue
                doc_means.append(enc.mean(axis=0))
        if doc_means:
    ...
    def initialize(self):
        ...
        for doc_num, doc in enumerate(pbar(self.corpus)):
            ...
            sentence_encodings = self._encoded_doc(doc_num, doc)
        ...
        ave_ll = self._compute_average_log_likelihood()
    ...
    def _encoded_doc(self, doc_num: int, doc) -> np.ndarray:
        if self.preencode_corpus:
            return self.encoded_corpus[doc_num]
    ...
    def _compute_average_log_likelihood(self) -> float:
        ...
        return calculate_sentence_gaussianlda_avg_ll(
    ...
    def update_table_params_chol(self, table_id, encoding, is_removed=False):
        ...
        if is_removed:
            ...
            x = (encoding - self.table_means[table_id]) * np.sqrt((k_n + 1.0) / k_n)
            ...
            x = (encoding - self.table_means[table_id]) * np.sqrt(k_n / (k_n - 1.0))
    ...
    def sample(self, num_iterations):
        ...
        for iteration in range(num_iterations):
            ...
            for d, doc in enumerate(pbar(self.corpus)):
                ...
                sentence_encodings = self._encoded_doc(d, doc)
                uniforms = np.random.random(len(sentence_encodings)).astype(
                    np.float64,
                    copy=False,
                )
